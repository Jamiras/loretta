...
parser.add_option('-f', '--font', action='store', dest='font_path', help='dump font to file')
...
parser.add_option('-s', '--screen', action='store', dest='screen', help='dump screen to png')
...
if options.screen:
    ...
    screen = Screen()
    screen.readtiles(options.charmap_path, 1)
    screen.dump_to_png(options.screen, rom, address, 32, count)


elif options.font_path:
    if options.png == True:
        TileTable.dump_to_png(rom, address, Format.RLE_8x8_1BPP, options.font_path, count)
    else:
        TileTable.dump_to_text(rom, address, Format.RLE_8x8_1BPP, options.font_path, count)


elif options.text_path:
    if not options.charmap_path:
        printf("--charmap is required for --text")
        exit()
